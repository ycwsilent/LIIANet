...
from net.LIIANet import LIIANet
...
eval_parser.add_argument('--test', action='store_true', help='my test')
eval_parser.add_argument('--weight_path', type=str,default='./weights/train/epoch_200.pth', help='my test weight_path')
...
def eval(model, testing_data_loader, model_path, output_folder,norm_size=True,LOL=False,v2=False,unpaired=False,alpha=1.0,gamma=1.0):
    ...
    for batch in tqdm(testing_data_loader):
        with torch.no_grad():
            ...
            input = input.cuda()
            output_rgb_mid,output = model(input**gamma) 
